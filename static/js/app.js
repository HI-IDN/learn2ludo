...
function updateMoveJustificationSettingsVisibility(){
  ...
  const everyRow=document.getElementById('move-justification-every-row');
  const sporadicRow=document.getElementById('move-justification-sporadic-row');
  if(everyRow) everyRow.hidden = mode !== 'every-n';
  if(sporadicRow) sporadicRow.hidden = mode !== 'random';
}
function saveSettings(){
  ...
  const sporadicChanceRaw = parseInt(document.getElementById('move-justification-sporadic-chance')?.value || 35);
  const sporadicChance = Number.isFinite(sporadicChanceRaw) ? Math.max(0, Math.min(100, sporadicChanceRaw)) : 35;
  settings={...settings,
...
function switchTab(id){ document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active')); document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active')); document.getElementById('tab-btn-'+id)?.classList.add('active'); document.getElementById('panel-'+id)?.classList.add('active'); if(id==='play'&&typeof moveReplaySurfaceToPlay==='function')moveReplaySurfaceToPlay(); if(id==='stats')loadStats(); if(id==='lobby')renderLobbySlots(); if(id==='profiles'&&typeof renderProfiles==='function')renderProfiles(); if(id==='bots')renderBotsPage(); if(id==='replays'){ if(typeof loadReplaysPage==='function')loadReplaysPage(); if(typeof moveReplaySurfaceToReplays==='function'&&typeof isReplayActive==='function'&&isReplayActive())moveReplaySurfaceToReplays(); } if(id==='howto'&&typeof howtoUpdateRlVisibility==='function')howtoUpdateRlVisibility(); }
...
async function makeMove(pieceIdx,target,pawnIdValue=null,justification=null){ window._botChosenMove=null;
  ...
  if(typeof moveNeedsJustification==='function'&&moveNeedsJustification(selectedMove,justification)){ requestMoveJustification(selectedMove); return; }
...
function renderGame(){
  if(!gameState)return;
  if(typeof renderMoveJustificationPrompt==='function') renderMoveJustificationPrompt();
